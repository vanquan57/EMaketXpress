    ...
    <div class="bg-[#F8F8F8]">
        <div class="grid lg:grid-cols-12 grid-cols-1 gap-6 lg:m-10 ">
            <div class="lg:col-span-8 col-span-1 bg-[#FFFFFF]   rounded-md mt-3">
                ...
                <div class="lg:hidden">
                            ...
                            <div class="flex-1 pl-2 group_UpAndDown_Number">
                                ...
                                <p class="font-medium text-[#FF5500] py-1">
                                    {{ number_format($productOfUser->Price_sale, 0, '.', '.') }} VNĐ<s
                                        class="text-[#8A8A8F]">{{ number_format($productOfUser->Price, 0, '.', '.') }}</s>
                                </p>
                                ...
                                <div class="flex justify-between mt-2">
                                    <div class="flex">
                                        ...
                                        <span
                                            class="border w-[40px] text-center flex items-center justify-center numberProduct"
                                            onclick="updateNumberProductInCart(this, {{ $productOfUser->pivot->ProductID }})">{{ $productOfUser->pivot->ProductNumbers }}</span>
                                        <button class="px-2 rounded-r-full border buttonIncrease">+</button>
                                    </div>
                                    <button id="btnRemoveProductInCart"
                                        onclick="removeRowProductInCart({{ $productOfUser->pivot->Product_cartID }})"
                    ...
                    <div class="pr-2 text-right">
                        Tổng cộng: <span
                            class="font-medium text-[#FF5500]">{{ number_format($totalProductsInCart, 0, '.', '.') }}</span>
                    </div>
                </div>
                                    ...
                                    <tr id="product_cartID_{{ $productOfUser->pivot->Product_cartID }}"
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="flex group_UpAndDown_Number justify-center">
                                                <button class="px-2 rounded-l-full border buttonReduced">-</button>
                                                <span class="border w-[40px] text-center  numberProduct"
                                                    onclick="updateNumberProductInCart(this, {{ $productOfUser->pivot->ProductID }})">{{ $productOfUser->pivot->ProductNumbers }}</span>
                                                <button class="px-2 rounded-r-full border buttonIncrease">+</button>
                                            </div>
                                        </td>
            ...
            <div class="bg-[#FFFFFF] lg:col-span-4 col-span-1 rounded-md mt-3 ">
                <div class="bg-[#EAEAF4] max-h-[300px] overflow-y-auto rounded-lg">
                    @foreach ($promotions as $promotion)
                        <div class="px-3 py-2 text-center group_Code_Promotion border ">
                            ...
                            <p class="font-light my-2">*Áp dụng cho sản phẩm toàn ngành hàng giảm
                                    {{ $promotion->DiscountAmount }} {{$promotion->DiscountType == 1 ? 'VNĐ' : '%'}}
                ...
                <a href="/cart/verification-order"
