...
@section('content')
    ...
    <div class="lg:max-w-[1200px] mx-auto">
        ...
        <div class="grid gap-5 grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
            @for ($i = 0; $i < $listProductSale5->count(); $i++)
                <div class="col-span-1 my-5 group_Product ">
                    <a href="{{ $listProductSale5[$i]->Slug }}" class="inline-block overflow-hidden relative rounded">
                        <img src="/storage/uploads/{{ $listProductSale5[$i]->Img }}" alt=""
                    ...
                    <div>
                        <div class="h-10">
                            <a href="" class="text-sm font-normal">{{ $listProductSale5[$i]->Name }}</a>
                        </div>
                        <div class="py-1">
                            <span
                                class="font-medium text-[#CD151C]">{{ number_format($listProductSale5[$i]->Price_sale, 0, ',', '.') . 'đ' }}</span>
                            <span
                                class="font-medium text-[#8A8A8F]"><s>{{ number_format($listProductSale5[$i]->Price_sale, 0, ',', '.') . 'đ' }}</s></span>
                        </div>
                    </div>
                    <ul class="flex item_image">
                        @for ($j = 0; $j < $imagesSale5->count(); $j++)
                            @if ($listProductSale5[$i]->ProductID == $imagesSale5[$j]->ProductID)
                                <li
                                    ...
                                    <img class="h-[100%] w-[100%] object-cover rounded-full"
                                        src=" /storage/uploads/{{ $imagesSale5[$j]->Img }}" alt="">
                                </li>
                            @endif
                        @endfor
                    </ul>
